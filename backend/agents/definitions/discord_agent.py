...
| hl-bot-claudeAI-01  | discord-claude-bot.service | Claude Bridge, Flask :8080 |
...
TOKEN=$(grep "^DISCORD_TOKEN_CLAUDE=" /opt/discord-bot/config/.env | cut -d= -f2)
...
- Footer: "Homelab HQ · DATUM · hl-bot-claudeAI-01"
...
```bash
systemctl status discord-bot discord-claude-bot discord-mod-bot --no-pager
# Logs wenn ein Bot down ist:
journalctl -u discord-claude-bot -n 50 --no-pager
# Bot neu starten:
systemctl restart discord-claude-bot
```
